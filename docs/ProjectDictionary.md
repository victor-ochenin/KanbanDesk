...
Callback — функция обратного вызова. Это функция, которая передаётся в другую функцию в качестве аргумента и вызывается после завершения какого-либо действия или события.

AlertDialog — класс для создания диалоговых окон с сообщениями, запросами ввода или подтверждением действий. Позволяет создавать модальные окна с заголовком, текстом, кнопками и кастомным View.